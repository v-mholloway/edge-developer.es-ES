...
1.  [Abra DevTools][DevToolsOpen] presionando `Control` + `Shift` + `J` \ (Windows, Linux \) o `Command` + `Option` + `J` \ (MacOS \).  Se abre el panel de la **consola** .  
...
1.  Haga clic con el botón derecho en el encabezado de la tabla registro de red y elija **dominio**.  Ahora se muestra el dominio de cada recurso.  
