...
Las comunicaciones en tiempo real \ (ORTC \) permite que los medios \ (audio y/o video \) se transtransmitan \ (enviados y recibidos \) en tiempo real directamente entre exploradores Web, dispositivos móviles y servidores a través de API de JavaScript nativas.  Consulta el tema de la guía de desarrollo [API de RTC](https://ortc.org) para obtener más información sobre ORTC en Microsoft Edge.  
