...
*   Se [agregó el método SetVirtualHostNameToFolderMapping][ReferenceWin32Icorewebview210781PreReleaseSetVirtualHostNameToFolderMapping] que asigna un nombre de host virtual a una ruta de directorio.  
...
*   Se agregó [la propiedad UserAgent][ReferenceWin32Icorewebview2experimentalsettings10781PreReleaseGetUserAgent] para obtener o establecer el agente de usuario. \([\#122][GithubMicrosoftedgeWebviewfeedbackIssue549]\)
...
*   Activado el diseñador winForms en .NET Core 3.1+ y .NET 5.  
...
*   Se actualizó la propiedad De origen del diseñador [de][DotnetApiMicrosoftWebWebview2WinformsWebview2Source] WinForms a la predeterminada o se restablece a null.  \([\#177][GithubMicrosoftedgeWebviewfeedbackIssue177]\).  
...
*   Se `ICoreWebView2NewWindowRequestedEventArgs` actualizó [para incluir la propiedad WindowFeatures][ReferenceWin32Icorewebview2newwindowrequestedeventargsGetWindowfeatures09622] y el [ICoreWebView2WindowFeatures asociado.][ReferenceWin32Icorewebview2windowfeatures09622]  \([\#293][GithubMicrosoftedgeWebviewfeedbackIssue293]\).  
...
*   Se ha asegurado de que se ejecuta el evento de inicio de navegación WebView para `about:blank` .  Ahora, los eventos se ejecutan para toda la navegación, pero las cancelaciones para o iframe no `NavigationStarting` `about:blank` se `srcdoc` admiten ni se omiten.  
...
*   Se agregó compatibilidad para el tipo de diálogo para eventos de diálogo `beforeunload` de JavaScript y se CORE_WEBVIEW2_SCRIPT_DIALOG_KIND_BEFOREUNLOAD entrada de enumeración. [][ReferenceWin32Icorewebview2CoreWebview2ScriptDialogKind09430]  
...
*   Se ha agregado para el script en ejecución en WebView2 para acceder a un objeto IDispatch desde el componente Win32 de la aplicación y acceder a las propiedades del objeto `Native Object Injection` IDispatch .  Para obtener más información, vaya [a AddRemoteObject][ReferenceWin32Iwebview2webview4Addremoteobject08190] \([#17][GithubMicrosoftedgeWebviewfeedbackIssue17]\).  
